...
pub struct PangleAdapter { pub endpoint: String }
impl PangleAdapter { pub fn new(endpoint: String) -> Self { Self { endpoint } } }
...
fn get_media_type_for_ad_type(ad_type: i32) -> Result<BidType, BidderError> {
    match ad_type {
        ...
        _ => Err(BidderError::BadServerResponse("unrecognized adtype in response".to_string())),
    }
}
...
impl Bidder for PangleAdapter {
    fn make_requests(&self, request: &openrtb::BidRequest, _: &ExtraRequestInfo) -> (Vec<RequestData>, Vec<BidderError>) {
        ...
        for imp in &request.imp {
            ...
            let is_rewarded = imp.ext.as_ref()
            ...
            if has_app != has_placement {
                errs.push(BidderError::BadInput("only one of appid or placementid is provided".to_string()));
                continue;
            }
            ...
            // Also carry through prebid ext if present
