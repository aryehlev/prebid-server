...
pub struct MediafuseAdapter { pub endpoint: String }
impl MediafuseAdapter { pub fn new(endpoint: String) -> Self { Self { endpoint } } }

#[derive(Serialize, Deserialize, Default)]
struct ExtImpMediafuse {
...
struct ImpExt {
    #[serde(default)]
    bidder: ExtImpMediafuse,
}

impl Bidder for MediafuseAdapter {
    fn make_requests(&self, request: &openrtb::BidRequest, _: &ExtraRequestInfo) -> (Vec<RequestData>, Vec<BidderError>) {
        ...
        for imp in &request.imp {
            let imp_ext: ImpExt = match imp.ext.as_ref().and_then(|e| serde_json::from_value(e.clone()).ok()) {
                Some(e) => e,
                None => {
                    errs.push(BidderError::BadInput("Failed to parse mediafuse imp ext".to_string()));
                    continue;
                }
            };
            ...
            if !member_id.is_empty() {
                if unique_member_id.is_empty() {
                    ...
                    errs.push(BidderError::BadInput(format!(
                        "all request.imp[i].ext.prebid.bidder.mediafuse.member params must match. Request contained member IDs {} and {}",
                        unique_member_id, member_id
                    )));
                    return (vec![], errs);
                }
