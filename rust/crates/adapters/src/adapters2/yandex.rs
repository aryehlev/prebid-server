...
mod tests {
        ...
        openrtb::BidRequest {
            ...
            imp: vec![openrtb::Imp {
                ...
                ext: Some(serde_json::json!({
                    "bidder": {"placementId": "R-A-123456-1"}
                })),
                ..Default::default()
            }],
    ...
    #[test]
    fn test_yandex_url_and_headers() {
        let adapter = YandexAdapter::new("https://bs.yandex.ru/metadsp/{{.PageID}}".to_string());
        let (reqs, errs) = adapter.make_requests(&make_req(), &ExtraRequestInfo::default());
        assert!(errs.is_empty(), "errs: {:?}", errs);
        assert_eq!(reqs.len(), 1);
        assert!(
            reqs[0].uri.starts_with("https://bs.yandex.ru/metadsp/123456"),
            "uri: {}",
            reqs[0].uri
        );
        assert_eq!(
        ...
        assert_eq!(
            reqs[0].headers.get("X-OpenRTB-Version").unwrap(),
            "2.5"
        );
    ...
    #[test]
    fn test_yandex_make_bids() {
        let adapter = YandexAdapter::new("https://bs.yandex.ru/metadsp/{{.PageID}}".to_string());
        let body = br#"{"id":"r","seatbid":[{"bid":[{"id":"b","impid":"i1","price":2.1,"crid":"c"}]}]}"#;
