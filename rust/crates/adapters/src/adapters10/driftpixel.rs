...
pub struct DriftpixelAdapter { pub endpoint: String }
impl DriftpixelAdapter { pub fn new(endpoint: String) -> Self { Self { endpoint } } }
...
impl Bidder for DriftpixelAdapter {
        ...
        for imp in &request.imp {
            ...
            if env.is_empty() || pid.is_empty() {
                errs.push(BidderError::BadInput(
                    "Failed to deserialize DriftPixel extension: missing env or pid".to_string()
                ));
                continue;
            }
