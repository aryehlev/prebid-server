...
pub struct BoldwinRapidAdapter { pub endpoint: String }
impl BoldwinRapidAdapter { pub fn new(endpoint: String) -> Self { Self { endpoint } } }
...
impl Bidder for BoldwinRapidAdapter {
    fn make_requests(&self, request: &openrtb::BidRequest, _: &ExtraRequestInfo) -> (Vec<RequestData>, Vec<BidderError>) {
        ...
        for imp in &request.imp {
            ...
            let pid = bidder.get("pid").and_then(|v| v.as_str()).unwrap_or("").to_string();
            let tid = bidder.get("tid").and_then(|v| v.as_str()).unwrap_or("").to_string();

            // boldwin_rapid uses {{.PublisherID}} for pid, {{.PlacementID}} for tid
            let url = self.endpoint
                .replace("{{.PublisherID}}", &pid)
                .replace("{{.PlacementID}}", &tid);
            ...
            headers.insert("Host".to_string(), "rtb.beardfleet.com".to_string());
