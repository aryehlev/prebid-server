...
impl Bidder for VideoHeroesAdapter {
    fn make_requests(&self, request: &openrtb::BidRequest, _: &ExtraRequestInfo) -> (Vec<RequestData>, Vec<BidderError>) {
        ...
        let ext: ExtImpVideoHeroes = match serde_json::from_value(bidder_ext.bidder) {
            Ok(v) => v,
            Err(_) => return (vec![], vec![BidderError::BadInput("ext.bidder not provided".to_string())]),
        };

        let url = self.endpoint.replace("{{.PublisherID}}", &ext.placement_id);
        ...
        (vec![RequestData { method: "POST".to_string(), uri: url, body, headers, imp_ids }], vec![])
    }
