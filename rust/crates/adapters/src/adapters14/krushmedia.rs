...
mod tests {
    ...
    #[test]
    fn test_make_requests_replaces_account_macro() {
        let adapter = KrushmediaAdapter::new("https://krushmedia.example/{{.AccountID}}/rtb".to_string());
        let mut req = openrtb::BidRequest::default();
        req.id = "r".to_string();
        req.imp = vec![openrtb::Imp {
            id: "imp1".to_string(),
            banner: Some(Default::default()),
            ext: Some(serde_json::json!({"bidder":{"accountId":"acc1"}})),
            ..Default::default()
        }];
        ...
        assert_eq!(requests[0].uri, "https://krushmedia.example/acc1/rtb");
