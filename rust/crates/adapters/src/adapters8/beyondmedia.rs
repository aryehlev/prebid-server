...
pub struct BeyondmediaAdapter {
    pub endpoint: String,
}

impl BeyondmediaAdapter {
...
impl Bidder for BeyondmediaAdapter {
    fn make_requests(&self, request: &openrtb::BidRequest, _: &ExtraRequestInfo) -> (Vec<RequestData>, Vec<BidderError>) {
        ...
        for imp in &request.imp {
            ...
            let bm_ext: ImpExtBeyondMedia = match serde_json::from_value(bidder_ext.bidder) {
                Ok(v) => v,
                Err(e) => { errs.push(BidderError::BadInput(e.to_string())); return (results, errs); }
            };

            let final_ext = match serde_json::to_value(ReqBodyExt {
                bidder: ReqBodyExtBidder {
                    typ: "publisher".to_string(),
                    placement_id: bm_ext.placement_id,
                },
            }) {
                Ok(v) => v,
                Err(e) => { errs.push(BidderError::BadInput(e.to_string())); return (results, errs); }
