...
impl Bidder for CompassAdapter {
    fn make_requests(&self, request: &openrtb::BidRequest, _: &ExtraRequestInfo) -> (Vec<RequestData>, Vec<BidderError>) {
        ...
        for imp in &request.imp {
            let ext_val = match &imp.ext {
                Some(v) => v.clone(),
                None => { errs.push(BidderError::BadInput("missing imp ext".to_string())); return (results, errs); }
            };
            ...
            let compass_ext: ImpExtCompass = match serde_json::from_value(bidder_ext.bidder) {
                Ok(v) => v,
                Err(e) => { errs.push(BidderError::BadInput(e.to_string())); return (results, errs); }
            };

            let ext_bidder = if !compass_ext.placement_id.is_empty() {
                ReqBodyExtBidder {
                    typ: "publisher".to_string(),
                    placement_id: compass_ext.placement_id,
                    endpoint_id: String::new(),
                }
            } else if !compass_ext.endpoint_id.is_empty() {
                ReqBodyExtBidder {
                    typ: "network".to_string(),
                    placement_id: String::new(),
                    endpoint_id: compass_ext.endpoint_id,
                }
            } else {
                ReqBodyExtBidder {
                    typ: String::new(),
