...
mod tests {
    ...
    #[test]
    fn test_make_requests_replaces_zone_macro() {
        let adapter = MadvertiseAdapter::new("https://madvertise.example/{{.ZoneID}}/rtb".to_string());
        let mut req = openrtb::BidRequest::default();
        req.id = "r".to_string();
        req.imp = vec![openrtb::Imp {
            id: "imp1".to_string(),
            banner: Some(Default::default()),
            ext: Some(serde_json::json!({"bidder":{"zoneId":"zone123"}})),
            ..Default::default()
        }];
        ...
        assert_eq!(requests[0].uri, "https://madvertise.example/zone123/rtb");
    }

    #[test]
    fn test_make_requests_zone_too_short() {
        let adapter = MadvertiseAdapter::new("https://madvertise.example/{{.ZoneID}}/rtb".to_string());
        let mut req = openrtb::BidRequest::default();
        req.imp = vec![openrtb::Imp {
            id: "imp1".to_string(),
            banner: Some(Default::default()),
            ext: Some(serde_json::json!({"bidder":{"zoneId":"short"}})),
            ..Default::default()
        }];
        let info = ExtraRequestInfo::default();
        let (requests, errs) = adapter.make_requests(&req, &info);
