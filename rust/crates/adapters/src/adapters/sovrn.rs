...
mod tests {
    ...
    fn make_req() -> openrtb::BidRequest {
        openrtb::BidRequest {
            ...
            imp: vec![openrtb::Imp {
                ...
                ext: Some(serde_json::json!({"bidder": {"tagid": "tag1"}})),
                ..Default::default()
            }],
            ..Default::default()
        }
    ...
    #[test]
    fn test_sovrn_url_and_headers() {
        let adapter = SovrnAdapter::new("http://ap.lijit.com/rtb/bid?src=prebid_server".to_string());
        let (reqs, _) = adapter.make_requests(&make_req(), &ExtraRequestInfo::default());
        assert_eq!(reqs.len(), 1);
        assert_eq!(reqs[0].uri, "http://ap.lijit.com/rtb/bid?src=prebid_server");
        assert_eq!(reqs[0].headers.get("Content-Type").unwrap(), "application/json");
        assert!(!reqs[0].body.is_empty());
    }

    #[test]
    fn test_sovrn_make_bids() {
        let adapter = SovrnAdapter::new("http://ap.lijit.com/rtb/bid?src=prebid_server".to_string());
        let body = br#"{"id":"r","seatbid":[{"bid":[{"id":"b","impid":"i1","price":0.5,"crid":"c"}]}]}"#;
        ...
        assert_eq!(result.bids[0].bid.price, 0.5);
    }
}
