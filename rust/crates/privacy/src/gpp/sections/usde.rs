//! US Delaware (DPDPA) GPP section — core segment parser.
//!
...
//! Specifically the `US-DE` section specification.
//!
//! Delaware's DPDPA follows the USNat baseline for the sensitive-data
//! category list. Per the IAB `US-DE` spec, the sensitive-data
...
/// Parsed core-segment fields of a US-DE GPP section.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct UsDeSection {
    pub version: u8,
    ...
    /// 8 two-bit values in DE (see module doc).
...
/// DPDPA defines 8 sensitive-data categories in the IAB US-DE section.
...
/// Parse the core segment of a US-DE GPP section from raw bytes.
pub fn parse(bytes: &[u8]) -> Result<UsDeSection, GppError> {
    if bytes.is_empty() {
        return Err(GppError::BadBitfield("empty UsDe payload".into()));
    }
