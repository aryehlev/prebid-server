//! US Florida (FDBR) GPP section — core segment parser.
//!
...
//! Specifically the `US-FL` section specification.
//!
//! Florida's FDBR follows the USNat baseline for the sensitive-data
//! category list. Per the IAB `US-FL` spec, the sensitive-data
...
/// Parsed core-segment fields of a US-FL GPP section.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct UsFlSection {
    pub version: u8,
    ...
    /// 8 two-bit values in FL (see module doc).
...
/// FDBR defines 8 sensitive-data categories in the IAB US-FL section.
...
/// Parse the core segment of a US-FL GPP section from raw bytes.
pub fn parse(bytes: &[u8]) -> Result<UsFlSection, GppError> {
    if bytes.is_empty() {
        return Err(GppError::BadBitfield("empty UsFl payload".into()));
    }
...
mod tests {
    ...
    fn parse_all_zero_payload() {
        // 6 bits version + (5 + 8 + 2 + 4) two-bit fields.
