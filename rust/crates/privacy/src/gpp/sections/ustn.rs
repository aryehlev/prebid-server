...
//! Specifically the `US-TN` section specification.
//!
//! Tennessee's TIPA follows the USNat baseline for the sensitive-data
//! category list. Per the IAB `US-TN` spec, the sensitive-data
...
pub struct UsTnSection {
    ...
    /// 8 two-bit values in TN (see module doc).
...
/// TIPA defines 8 sensitive-data categories in the IAB US-TN section.
