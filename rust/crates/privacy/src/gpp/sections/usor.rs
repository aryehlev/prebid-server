...
/// Parsed core-segment fields of a US-OR GPP section.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct UsOrSection {
    pub version: u8,
...
pub fn parse(bytes: &[u8]) -> Result<UsOrSection, GppError> {
    if bytes.is_empty() {
        return Err(GppError::BadBitfield("empty UsOr payload".into()));
    }
