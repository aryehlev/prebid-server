//! US Montana (MCDPA) GPP section — core segment parser.
//!
...
//! Specifically the `US-MT` section specification.
//!
//! Montana's MCDPA follows the USNat baseline for the sensitive-data
//! category list. Per the IAB `US-MT` spec, the sensitive-data
...
/// Parsed core-segment fields of a US-MT GPP section.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct UsMtSection {
    pub version: u8,
    ...
    /// 8 two-bit values in MT (see module doc).
...
/// MCDPA defines 8 sensitive-data categories in the IAB US-MT section.
...
/// Parse the core segment of a US-MT GPP section from raw bytes.
pub fn parse(bytes: &[u8]) -> Result<UsMtSection, GppError> {
    if bytes.is_empty() {
        return Err(GppError::BadBitfield("empty UsMt payload".into()));
    }
