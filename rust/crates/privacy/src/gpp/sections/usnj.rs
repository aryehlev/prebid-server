...
/// Parsed core-segment fields of a US-NJ GPP section.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct UsNjSection {
    pub version: u8,
...
pub fn parse(bytes: &[u8]) -> Result<UsNjSection, GppError> {
    if bytes.is_empty() {
        return Err(GppError::BadBitfield("empty UsNj payload".into()));
    }
