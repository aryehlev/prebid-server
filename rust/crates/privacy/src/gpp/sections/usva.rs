//! US Virginia (VCDPA) GPP section — core segment parser.
//!
//! Uses USNat as the baseline. VA has a smaller sensitive-data
//! category list and no "sale" notion per se (only
//! targeted-advertising / sale-of-personal-data), but we keep the
//! struct uniform for consumer simplicity.
//!
...
//!   sensitive_data_processing            8 x 2  (VA: 8 categories)
...
//!
//! NOTE: Real VA section also includes a personal_data_consents field
//! positioned right before the MSPA triple; we keep it for struct
//! parity with other states.

use crate::gpp::bitfield::BitReader;
use crate::gpp::GppError;

/// Parsed core-segment fields of a US-VA GPP section.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct UsVaSection {
    pub version: u8,
    ...
    /// 8 two-bit values in VA.
    pub sensitive_data_processing: Vec<u8>,
    /// 1 two-bit value in VA.
...
/// Parse the core segment of a US-VA GPP section from raw bytes.
pub fn parse(bytes: &[u8]) -> Result<UsVaSection, GppError> {
    if bytes.is_empty() {
        return Err(GppError::BadBitfield("empty UsVa payload".into()));
    }
    ...
    // VA has no separate sharing_opt_out_notice; reported as 0.
    ...
    // VA has no separate sharing_opt_out; reported as 0.
...
mod tests {
    ...
    fn parse_all_zero_payload() {
        // 6 bits version + (5 + 8 + 1 + 4) two-bit fields = 6 + 36 = 42 bits.
    ...
    fn parse_known_values() {
        // version=2, sharing_notice=1, sale_opt_out_notice=2,
        // TA_notice=3, sale_opt_out=1, TA_opt_out=2,
        // sensitive data = [1,2,3,0,1,2,3,0], known_child=[1],
        // personal=2, mspa_covered=1, mspa_opt_mode=2, mspa_sp_mode=3
        ...
        assert_eq!(s.known_child_sensitive_data_consents, vec![1]);
        assert_eq!(s.personal_data_consents, 2);
        assert_eq!(s.mspa_covered, 1);
        assert_eq!(s.mspa_opt_out_option_mode, 2);
        assert_eq!(s.mspa_service_provider_mode, 3);
    }
