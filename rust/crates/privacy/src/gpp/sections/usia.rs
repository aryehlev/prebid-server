//! US Iowa (ICDPA) GPP section — core segment parser.
//!
...
//! Specifically the `US-IA` section specification.
//!
//! Iowa's ICDPA follows the USNat baseline for the sensitive-data
//! category list. Per the IAB `US-IA` spec, the sensitive-data
...
/// Parsed core-segment fields of a US-IA GPP section.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct UsIaSection {
    pub version: u8,
    ...
    /// 8 two-bit values in IA (see module doc).
...
/// ICDPA defines 8 sensitive-data categories in the IAB US-IA section.
...
/// Parse the core segment of a US-IA GPP section from raw bytes.
pub fn parse(bytes: &[u8]) -> Result<UsIaSection, GppError> {
    if bytes.is_empty() {
        return Err(GppError::BadBitfield("empty UsIa payload".into()));
    }
