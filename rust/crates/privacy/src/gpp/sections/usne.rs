//! US Nebraska (NDPA) GPP section — core segment parser.
//!
...
//! Specifically the `US-NE` section specification.
//!
//! Nebraska's NDPA follows the USNat baseline for the sensitive-data
//! category list. Per the IAB `US-NE` spec, the sensitive-data
...
/// Parsed core-segment fields of a US-NE GPP section.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct UsNeSection {
    pub version: u8,
    ...
    /// 8 two-bit values in NE (see module doc).
...
/// NDPA defines 8 sensitive-data categories in the IAB US-NE section.
...
/// Parse the core segment of a US-NE GPP section from raw bytes.
pub fn parse(bytes: &[u8]) -> Result<UsNeSection, GppError> {
    if bytes.is_empty() {
        return Err(GppError::BadBitfield("empty UsNe payload".into()));
    }
