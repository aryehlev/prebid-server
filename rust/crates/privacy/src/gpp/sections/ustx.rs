//! US Texas (TDPSA) GPP section — core segment parser.
//!
...
//! Specifically the `US-TX` section specification.
//!
//! Texas's TDPSA follows the USNat baseline for the sensitive-data
//! category list. Per the IAB `US-TX` spec, the sensitive-data
...
/// Parsed core-segment fields of a US-TX GPP section.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct UsTxSection {
    pub version: u8,
    ...
    /// 8 two-bit values in TX (see module doc).
...
/// TDPSA defines 8 sensitive-data categories in the IAB US-TX section.
...
/// Parse the core segment of a US-TX GPP section from raw bytes.
pub fn parse(bytes: &[u8]) -> Result<UsTxSection, GppError> {
    if bytes.is_empty() {
        return Err(GppError::BadBitfield("empty UsTx payload".into()));
    }
