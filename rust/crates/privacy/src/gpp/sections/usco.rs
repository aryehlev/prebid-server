//! US Colorado (CPA) GPP section — core segment parser.
//!
//! Uses USNat as the baseline; CO's spec is close to VA, with 7
//! sensitive-data categories and 1 known-child field. Where the real
//! bit layout would diverge from this simplified structure in
//! production, comments call it out.
//!
...
//!   sensitive_data_processing            7 x 2
...
/// Parsed core-segment fields of a US-CO GPP section.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct UsCoSection {
    pub version: u8,
    ...
    /// 7 two-bit values in CO.
    pub sensitive_data_processing: Vec<u8>,
    /// 1 two-bit value in CO.
...
const SENSITIVE_DATA_CATEGORIES: usize = 7;
const KNOWN_CHILD_FIELDS: usize = 1;

/// Parse the core segment of a US-CO GPP section from raw bytes.
pub fn parse(bytes: &[u8]) -> Result<UsCoSection, GppError> {
    if bytes.is_empty() {
        return Err(GppError::BadBitfield("empty UsCo payload".into()));
    }
...
mod tests {
    ...
    fn parse_all_zero_payload() {
        ...
        assert_eq!(s.sensitive_data_processing.len(), 7);
    ...
    fn parse_known_values() {
        let mut fields: Vec<(u64, usize)> = vec![
            (4, 6), // version
            (2, 2), // sharing_notice
            (1, 2), // sale_opt_out_notice
            (3, 2), // TA_notice
            (2, 2), // sale_opt_out
            (1, 2), // TA_opt_out
        ];
        for v in [1u64, 0, 2, 3, 1, 0, 2] {
            fields.push((v, 2));
        }
        fields.push((3, 2)); // known_child[0]
        fields.push((1, 2)); // personal_data_consents
        fields.push((2, 2)); // mspa_covered
        fields.push((3, 2)); // mspa_opt_out_option_mode
        fields.push((1, 2)); // mspa_service_provider_mode
        let bytes = pack(&fields);

        let s = parse(&bytes).unwrap();
        assert_eq!(s.version, 4);
        assert_eq!(s.sharing_notice, 2);
        ...
        assert_eq!(s.sensitive_data_processing, vec![1, 0, 2, 3, 1, 0, 2]);
        assert_eq!(s.known_child_sensitive_data_consents, vec![3]);
