//! US New Hampshire (NHPA) GPP section — core segment parser.
//!
...
//! Specifically the `US-NH` section specification.
//!
//! New Hampshire's NHPA follows the USNat baseline for the
//! sensitive-data category list. Per the IAB `US-NH` spec, the
//! sensitive-data processing field contains 8 two-bit values.
//!
...
/// Parsed core-segment fields of a US-NH GPP section.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct UsNhSection {
    pub version: u8,
    ...
    /// 8 two-bit values in NH (see module doc).
...
/// NHPA defines 8 sensitive-data categories in the IAB US-NH section.
...
/// Parse the core segment of a US-NH GPP section from raw bytes.
pub fn parse(bytes: &[u8]) -> Result<UsNhSection, GppError> {
    if bytes.is_empty() {
        return Err(GppError::BadBitfield("empty UsNh payload".into()));
    }
